// x402 licensing terms, surfaced in the UI.
//
// These values mirror the sibling service at ../x402-nft-licensing
// (REQUIRED_AMOUNT_WEI in api/index.js — 0.0001 ETH on Base, paid directly to the
// NFT's current holder). Nothing here performs a request yet: this pass presents
// licensing state and copy only. When the integration lands, `apiBase` is the
// single place the client needs to point at.
export const LICENSE = {
  protocol: 'x402',
  chain: 'Base',
  token: 'ETH',
  priceEth: '0.0001',
  // Unused until the x402 integration lands — see .env.example.
  apiBase: import.meta.env.VITE_X402_API_BASE ?? '',
};

export const LICENSE_STATUS = {
  LICENSABLE: 'licensable',
  PENDING: 'pending',
  DEMO: 'demo',
};

export const LICENSE_COPY = {
  [LICENSE_STATUS.LICENSABLE]: {
    label: `Licensable · ${LICENSE.priceEth} ETH`,
    detail: `Licensed on generation via ${LICENSE.protocol} on ${LICENSE.chain}. The fee goes to the piece's current holder.`,
  },
  [LICENSE_STATUS.PENDING]: {
    label: 'Licensing pending',
    detail: 'This brand is being onboarded. Its collection will be licensable soon.',
  },
  [LICENSE_STATUS.DEMO]: {
    label: 'Demo data',
    detail:
      'No Alchemy API key is configured, so these are placeholder pieces. Add VITE_ALCHEMY_API_KEY to load real collections.',
  },
};
